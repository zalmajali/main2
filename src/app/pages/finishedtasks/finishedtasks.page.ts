...
@Component({
  selector: 'app-finishedtasks',
  templateUrl: './finishedtasks.page.html',
  styleUrls: ['./finishedtasks.page.scss'],
})
export class FinishedtasksPage implements OnInit {
  ...
  async functionGetData(userId:any,getDataByUser:any,orderId:any=0,departmentId:any=0,assigned:any=0) {
    ...
    this.requestsService.finishedTasks(userId,getDataByUser,orderId,departmentId,assigned,limitNew).then(async data=>{
  ...
  async functionTaskDetails(taskId,index){
    this.navCtrl.navigateRoot(["/taskdetails", {taskId:taskId,page:5}]);
  }
  async functionTaskCorrespondence(taskId,index,requestId){
    this.navCtrl.navigateRoot(["/correspondencetasks", {requestId:requestId,taskId:taskId,page:5}]);
  }
