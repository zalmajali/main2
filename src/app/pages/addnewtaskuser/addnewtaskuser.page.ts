...
@Component({
  selector: 'app-addnewtaskuser',
  templateUrl: './addnewtaskuser.page.html',
  styleUrls: ['./addnewtaskuser.page.scss'],
})
export class AddnewtaskuserPage implements OnInit {
  public menu3:any;
  public menu5:any;
  public menu6:any;
  public menu7:any;
  ...
  public getOrders:any=2;
  public msgAddTaskForUser:any;
  ...
  initialiseTranslation(){
    ...
    this.translate.get('add_new_task_user').subscribe((res: string) => {
      this.addNewTask = res;
    });
    ...
    this.translate.get('msg_add_task_for_user').subscribe((res: string) => {
      this.msgAddTaskForUser = res;
    });
    //menue
    //last menue
  ...
  async ngOnInit() {
    ...
    this.activaterouter.params.subscribe(params => {
      this.suberViserSearchName = params['suberViserSearchName'];
      this.backePage = params['backPage'];
    });
    await this.functionInformation(this.suberViserSearchName);
    await this.requestsService.getAllRequestUserSelect(this.suberViserSearchName).then(async data=>{
  ...
  async addTask(){
    ...
    if(this.orderId != undefined && this.orderId != null) {
      ...
      await loading.present();
      this.requestsService.addTask(this.orderId,this.suberViserSearchName,this.userId).then(async data=>{
        this.returnData = data;
        this.operationResult = this.returnData.Error.ErrorCode;
        if(this.operationResult==1){
          this.message = this.addMessageSuccess;
          this.displayResult(this.message);
          this.navCtrl.navigateRoot(["/alltasks",{assigned:this.suberViserSearchName,page:this.backePage}]);
