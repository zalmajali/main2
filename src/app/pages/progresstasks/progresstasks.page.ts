...
@Component({
  selector: 'app-progresstasks',
  templateUrl: './progresstasks.page.html',
  styleUrls: ['./progresstasks.page.scss'],
})
export class ProgresstasksPage implements OnInit {
  ...
  async functionGetData(userId:any,getDataByUser:any,orderId:any=0,departmentId:any=0,assigned:any=0) {
    ...
    this.requestsService.progressTasks(userId,getDataByUser,orderId,departmentId,assigned,limitNew).then(async data=>{
  ...
  async functionTaskDetails(taskId,index){
    this.navCtrl.navigateRoot(["/taskdetails", {taskId:taskId,page:4}]);
  }
  async functionTaskCorrespondence(taskId,index,requestId){
    this.navCtrl.navigateRoot(["/correspondencetasks", {requestId:requestId,taskId:taskId,page:4}]);
  }
  async functionTaskHold(taskId,requestId,index){
  ...
  async functionTaskOperations(taskId,index){
    let model = await this.modalController.create({
  ...
  async functionTaskFinish(taskId,index){
    this.navCtrl.navigateRoot(["/taskfinished",{taskId:taskId,requestId:this.returnTasksArray[index]['requestId']}]);
  }
