...
@Component({
  selector: 'app-processingrequests',
  templateUrl: './processingrequests.page.html',
  styleUrls: ['./processingrequests.page.scss'],
})
export class ProcessingrequestsPage implements OnInit {
  ...
  async functionGetData(userId:any,departmentId:any,getDataByUser:any,orderId:any=0,attachments:any=0,fromSelectDate:any=0,toSelectDate:any=0,departmentSearchId:any=0,suberViserSearchName:any=0,machineSearchNumber:any=0,statusSearch:any=0,urgencySearch:any=0,machineSearchDescription:any=0,faultSearchDescription:any=0,sortingBy:any=0) {
    ...
    this.requestsService.processingRequest(userId,departmentId,getDataByUser,orderId,attachments,fromSelectDate,toSelectDate,departmentSearchId,suberViserSearchName,machineSearchNumber,statusSearch,urgencySearch,machineSearchDescription,faultSearchDescription,sortingBy,limitNew).then(async data=>{
  ...
  functionDetails(requestId:any,index:any){
    this.navCtrl.navigateRoot(["/requestdetails", {requestId:requestId,page:4}]);
  }
  //done
    ...
    model.onDidDismiss().then(data=>{
      ...
      if(this.cancelledreason!=undefined && this.cancelledreason!="" && this.cancelledreason!=0 && this.cancelledreason!=null){
        this.requestsService.requestCancel(requestId,this.cancelledreason,this.userId).then(async data=>{
          ...
          this.operationResult = this.returnRequestData.Error.ErrorCode;
          if(this.operationResult==1){
            this.returnRequestArray[index]['show'] = 4;
  ...
  functionCorrespondence(requestId:any,index:any){
    this.navCtrl.navigateRoot(["/correspondence", {requestId:requestId,page:4}]);
  }
  //done
  functionTasks(requestId:any,index:any){
    this.navCtrl.navigateRoot(["/alltasks", {requestId:requestId,page:4}]);
  }
//done
  functionFinish(requestId:any,index:any){
    this.requestsService.requestDetailsFineshed(this.userId,requestId).then(async data=>{
      ...
      this.operationResult = this.returnRequestData.Error.ErrorCode;
      if(this.operationResult==1){
        this.navCtrl.navigateRoot(["/requestsfinished", {requestId:requestId,page:4}]);
      }else{
